      ...
      {activeTab === "overview" && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
            <StatCard title="Total Expenses" value={profile?._count?.expenses || 0} icon="💸" color="#ff4757" />
            <StatCard title="Total Income" value={profile?._count?.incomes || 0} icon="💰" color="#2ed573" />
            <StatCard title="Categories" value={profile?._count?.categories || 0} icon="📊" color="#3742fa" />
            <StatCard title="Member Since" value={new Date(profile?.createdAt).toLocaleDateString()} icon="📅" color="#ffa502" />
          </div>
          ...
          <div className="p-6 bg-white rounded-lg shadow mb-5">
            ...
            <div className="flex flex-col gap-4">
              <InfoCard label="Email" value={user?.email || "N/A"} icon={mailIcon} />
              <InfoCard label="Account Created" value={new Date(profile?.createdAt).toLocaleString()} icon={calendarIcon} />
              <InfoCard label="Last Updated" value={profile?.updatedAt ? new Date(profile?.updatedAt).toLocaleString() : "N/A"} icon={clockIcon} />
            </div>
          </div>
        </div>
      )}
...
const StatCard = ({ title, value, icon, color }) => (
  <div className="p-5 bg-white rounded-lg shadow border-l-4" style={{ borderColor: color }}>
...
// MODIFIÉ: Composant InfoCard pour inclure une icône
const InfoCard = ({ label, value, icon }) => (
  <div className="p-5 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 flex items-center space-x-4">
    {icon}
